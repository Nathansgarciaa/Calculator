...
# --- Evaluation to beta-normal form (normal-order) ---
...
def evaluate(tree):
    ...
    if tag == 'lam':
        # Reduce under lambda (normal form requires this)
    ...
    if tag == 'app':
        ...
        if func[0] == 'lam':
            ...
            name = func[1]
            body = func[2]
            # IMPORTANT: DO NOT evaluate argument before substitution
            return evaluate(substitute(body, name, tree[2]))

        # If function is NOT a lambda, normal form says:
        # app(e1,e2) is in normal form IF e1 is not lambda
        # BUT we still need to reduce e2 to normal form
        arg_nf = evaluate(tree[2])
        return ('app', func, arg_nf)
